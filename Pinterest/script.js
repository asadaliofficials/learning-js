...
function showTheCards(){
  ...
  arr.forEach(function(obj){
    allPostsData += `     <div class="box">
                      <img class="cursor-pointer" src="${obj.image}">
                      <div class="caption">${obj.name}</div>
                    </div>`
  })

  document.querySelector(".container").innerHTML = allPostsData;
}
showTheCards()



//  Show overlay on search Click 
...
// Show Search Suggestion on input by filter method

function showSuggestions(){
   input.addEventListener("input", function(){
    if(input.value){
       document.querySelector(".searchdata").style.display = "block"
       let filteredData = arr.filter(obj =>
        obj.name.toLowerCase().includes(input.value)
      );
      let searchData = ""
      ...
      if(filteredData.length == 0){
        searchData += `<div class="res flex px-8 py-3">
        <h3 class="font-semibold">Nothing to show here...</h3>
        </div>`
      }
